...
**Still flagged, not fixed: COMO Parrot Cay** holds `city: "Providenciales"` with its own cay in the area field, while Pine Cay (1259) puts the cay in `city`. Two private-island rows disagreeing; needs the same per-row call.
...
**Changing a `city` means rebuilding `cityAirports.ts` in the same pass** (§37, §43, and §49's "geography values are join keys, not display strings"). Every old value was a live key there. The first rebuild swapped three, 514 → 513 cities; the second removed `Majorca`, `Salzburg` and `Calvia` and added `Calvià` and `Hof bei Salzburg`, 513 → 512 — one fewer because two groups merged into one town. Merging also moved that centroid, so `Calvià`'s distance to PMI went 23 → 19km. Etéreo joining `Riviera Maya` moved that group's centroid and earned it **CUN at 33km**, which it had not been offered before. Verify after: every published `city` must have an airport entry, or the landing flight teaser resolves it to nothing.
