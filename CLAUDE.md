...
**Still open** (from the same screen): 10 rows repeat the city verbatim — six Phinda lodges, Ngala, Suyian, two Anguilla bays — and three cities disagree with themselves: New York carries `Midtown`, `Midtown East`, `Midtown West` *and* `Midtown Manhattan`, Venice `Giudecca` and `Giudecca Island`, London `Mayfair` plus two compound variants. Those are a granularity decision, not a correction, and it sets the pattern for whoever fills Bangkok's 14 empty rows.
