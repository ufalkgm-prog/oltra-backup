...
**Two Dubai rows are flagged, not fixed.** `1614` The Ritz-Carlton, Al Wadi Desert carries `city: "Dubai"` with coordinates in **Ras Al Khaimah, a different emirate ~90km away** — giving it a Dubai district would have buried a `city` error under a `local_area` one, and fixing it needs the `cityAirports.ts` rebuild any city change needs. And `2039` holds `Dubai International Financial Centre (DIFC)`, a parenthetical where every sibling is a bare name; not wrong, just the only verbose one.
...
* **Rosewood Schloss Fuschl** — `city` "Salzburg" → **"Hof bei Salzburg"**, its own municipality ~17km east; `local_area` cleared, since it would now echo `city`; and `area` filled with **"Salzkammergut"**. The area is populated rather than left null because §3 only empties it for a *major city* where `city` does the job — nobody searches "Hof bei Salzburg". Lech Am Arlberg is the same shape, a village with area "Arlberg". Corroboration that the city was wrong was already in the generated file: `cityAirports.ts` had measured that "Salzburg" centroid at **19km from SZG**, which a hotel in Salzburg never is.
* **Mandarin Oriental, Mallorca** — `city` "Majorca" → **"Calvià"**, `local_area` cleared. The other eight Balearic rows are unanimous: area is the island, `city` is the actual town (Deia, Puigpunyent, Banyalbufar, Calvia), so only this row departed from the house pattern. **Castell Son Claret's `Calvia` was accented to match** — not a defect alone, but leaving the pair would make one municipality two city keys, and §49's rule is that stripping an accent is a data downgrade.

Neither gained a `local_area`: Punta Negra is a headland and Lake Fuschl a lake, and neither village has districts.

**COMO Parrot Cay was the last of them**, fixed the same day: `city` "Providenciales" → **"Parrot Cay"**, matching Pine Cay (1259), which already put its own cay in `city`. The whole Turks & Caicos group duplicates `area` into `city`, because an island of one hotel has no town below it. The coordinates settle it — Parrot Cay is ~15km northeast of Grace Bay and nearer Pine Cay than any Providenciales hotel — and its own description says Providenciales is where you *transit*: "reached by road transfer to Leeward Marina on Providenciales followed by a boat crossing". **A gateway is not an address**, the same confusion that put an airport name in the destination field (§39).
