...
* **signup** — password ≥7 chars with letters and numbers. **Supabase does NOT tell us an address is already registered.** An earlier version of this line said it "detects duplicate emails natively"; it does not. Probed 2026-09-21: `mailer_autoconfirm: false`, so email confirmation is on, and with it on Supabase answers a signup for an existing confirmed address with a *decoy* user and **no error** — deliberately, so this form cannot be used to discover who has an account. A second account is never created; uniqueness is enforced in `auth.users`. The cost is that such a visitor is told to check their mail and none arrives, so the confirmation screen names the way out (log in, or reset) **for everyone**, because saying it only to them leaks exactly what the decoy protects.
* **after signup** — the form comes down and the panel shows only the confirmation and an OK button back to the landing page (Ulrik, 2026-09-21). `signUp` now sends `emailRedirectTo` at `/auth/callback?next=/members`, so confirming the address signs the member in and lands them on Members; the callback also creates their `member_profiles` row, which used to appear only when they saved Personal Information. **The mail's subject and body are Supabase dashboard templates, not in this repo** — Authentication → Email Templates → Confirm signup.
...
| City → airport mapping (§37) | When the roster's city list changes | 2026-09-23 | on demand | `build-city-airports.mjs` |
