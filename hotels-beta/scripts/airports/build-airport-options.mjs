...
// Server-only companion: each selectable airport's coordinates, for the
// concierge's flying-time estimate. Kept out of airportOptions.ts, which the
// browser downloads.
const COORDS_OUTPUT_PATH = path.join(REPO_ROOT, "src", "lib", "airportCoords.ts");
...
async function main() {
  ...
  for (let i = 1; i < csvRows.length; i++) {
    ...
    const candidate = {
      ...
      lng: Number(r[idx.longitude_deg]),
    };
...
`;
  ...
  let coords = `// AUTO-GENERATED by scripts/airports/build-airport-options.mjs — do not
// hand-edit. Re-run that script to regenerate.
//
...
`;
  for (const o of [...options].sort((a, b) => a.value.localeCompare(b.value))) {
    if (!Number.isFinite(o.lat) || !Number.isFinite(o.lng)) continue;
    coords += `  ${o.value}: [${round(o.lat)}, ${round(o.lng)}],\n`;
  }
  coords += `};
`;
  fs.writeFileSync(COORDS_OUTPUT_PATH, coords);
  console.log(`Wrote ${COORDS_OUTPUT_PATH} — ${(fs.statSync(COORDS_OUTPUT_PATH).size / 1024).toFixed(0)} KB.`);
  console.log(
