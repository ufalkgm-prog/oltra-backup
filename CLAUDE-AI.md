...
**Back and Forward close the panel (Ulrik, 2026-09-21).** The note below assumed the modal always unmounts, and on history navigation it does not: `AiConciergeRoot` closes it only when the new path is off the allow-list, and the header links close it in an `onClick` that Back never runs. So Back between two allowed pages left the panel open over a page the visitor had moved to, the cleanup never ran, and `<html>`/`<body>` kept `overflow: hidden` - the page could not be scrolled at all. A `popstate` listener in the modal now closes it. **Deliberately `popstate` and not a pathname/searchParams watcher**: `AiResultsSync` writes an answer's own dates and ids into the URL, so closing on every URL change would shut the panel the moment it answered.
...
* **A big question outran the route (2026-09-16).** "Tokyo and Kyoto, two weeks, hotels, flights and restaurants" took **110s** locally; production's `maxDuration = 60` would have cut it off with the panel still on "Thinking…", which had no limit of its own. Now: `maxDuration = 180` (Ulrik rejected 300 as too long to wait); `abortSignal: req.signal`, so Stop ends the model call and its tools instead of letting them run on; the panel lists each search as it runs, read from the streaming tool parts; and it gives up itself at 170s overall or 90s with no chunk arriving, removing the turn (as a failed turn is) and putting the question back in the box. **Where the 110s went has not been measured yet** — run the `[perf]` logging above on that question before choosing between a faster `CHAT_MODEL`, a per-step model split via `prepareStep`, or parallelising our own tools.
* **Measured on a long question (2026-09-23): the model is the time, not our tools.** "New Year's Eve with fireworks from the hotel, four nights, from Copenhagen, under five hours" took 149s: triage 1s, six model steps of 15/18/39/15/25/31s, and our tools only ~25s of it (the slowest a 5-11s availability batch). Each step costs 10-30s of model time on its own; the last, writing presentResults, took 31s for 2,500 tokens (1,400 of them thinking), and one step read 101k tokens of candidate lists. **A faster model for the search steps was tried a few days earlier and saved no time (Ulrik) — do not propose it again.** What was built: searchHotels carries descriptions when it returns 8 or fewer (`INLINE_DESCRIPTIONS_MAX`, fetched beside the availability pass), which removed the separate getHotelDetails step (14.6s there, 0.3s of it lookups) — re-run 133s, five steps, no detail step, though the writing step grew to 39s. The panel also says what the searches have found so far, from the tool outputs ("Found so far: 23 hotels to consider in Positano, Taormina… 9 with rooms for 16 Apr – 19 Apr"), at no cost in time, and "This may take a couple of minutes." once a second model step runs searches or at 45s — the writing step does not count, or every two-step answer showed it. Then, with dates known, a hotel with no rates for them goes to the model as id, name and city only (`noRoomsForTheseDates`; "not-sold-here" keeps its record, since it is still recommended): the directory gate already counted only the free ones, but every candidate was sent in full. Re-run 115s, the first search 13,800 characters where that step had read 101k tokens. Still open if wanted: less thinking on intermediate steps.
