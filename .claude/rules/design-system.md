...
| Neutral | `--neutral` | **`#E3A877`** | **`#F0C29C`** | **pale orange since 2026-09-23 (Ulrik)** — the active grey (`#9AA39E`/`#CBD0CB`) read as passive; the Flights tabs keep that grey as their hover edge via `--oltra-choice-grey`. Leaving OLTRA for a hotel we cannot sell ("Book on website" — shortened from "Check availability on website" 2026-09-21), caveat inside the button. **These cards sort last in every list** (`sellableFirst`) |
...
The header "AI Concierge" is **transparent with a white rim and label**, no orange at all (`.oltra-btn--ai-concierge`). The landing field "Ask AI" is a **normal active button** — transparent, sage rim, standard label — at the AI variant's smaller size and still bold italic (`.oltra-btn--ai-ask`). Both are modifiers on `.oltra-btn--ai` in `oltra-theme.css`, applied by `AiModeButton`. "Become a member" keeps the plain orange pill. Everything above about the filled variant now describes only that button.
