...
* **One destination at a time; flights excepted (Ulrik, 2026-09-15).** The pages price one stay, so a Marrakech-then-Atlas answer priced the mountain hotel on the city's dates. `presentResults.laterStops` (`{place, checkIn, checkOut, hotelIds, restaurantIds}[]`) carries every place after the first; `hotelIds`/`restaurantIds`/`stay`/`destination` are the first place only and alone reach the store's cards. The panel lists each place under "Hotels in {place} · dates", and code writes Ulrik's sentence: "I can only show you details for one destination at a time. So, first I have listed the hotels in {first} {behind this window / on this page / on the main page}… ask me to list the hotels in {next}." Asking for the next place presents it with its own dates and clears the stops. Multi-leg flights are unaffected. Not for alternatives spread across places.
